...
namespace dbgen3
{
  ...
  private:
    ...
    static fld_vec    fetch_param_dscr(const str_vec& names, const db::BUF_TYPE& a_bt, db::statement& a_stmt);
    ...
    static gsql_qbuf_dscr load_buf(const db::BUF_TYPE& bt, const x::DOMElement* an_el);
    static gsql_sql_set   load_sql_set(const x::DOMElement* an_el, const str_vec& a_ctx, const RDBMS& a_db_type);
