...
TEST_CASE("T1 - basic generator test") 
{ 
  db::connection c("test");
  q_set_id::q_id1::qp_id<2> qp; 
  qp.set_par_1(10);
  qp.set_par_1(11,1);
  REQUIRE_EQ (qp.par_1(), 10);
  REQUIRE_EQ (qp.par_1(1), 11);
  ...
  auto date=SQL_DATE_STRUCT(2022,1,22);
  auto time=SQL_TIME_STRUCT(23,59,59);
  auto ts=SQL_TIMESTAMP_STRUCT(2022,1,22,23,59,59,99999);
  qr.set_c9_date(date);
